...
/**
 * Given tree below, create function with the following signature which finds closest common parent
 * of second and third parameter.
 * 
 * function (top_node, node1, node2)
 * where
 * 		top_node = highest node in tree
...
public class FindCommonParentInTree {
	...
	public class Node {
		...
		private String name;
		private List<Node> reportees;

		public Node(String name) {
			this.setName(name);
			reportees = new ArrayList<Node>();
		}

		public List<Node> getReportees() {
			return reportees;
		}
	...
	private static void searchTree(Node highestNode, Node topNode, Node node, List<Node> parents) {
		...
		for (Node n : topNode.getReportees()) {
